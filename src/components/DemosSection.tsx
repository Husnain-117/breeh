...
const demos = [
  {
    ...
    description: "Every inbound call answered instantly — triage, FAQs, and routing handled seamlessly across specialties.",
  ...
  },
  {
    ...
    description: "Track call volume, peak hours, patient sentiment, and booking conversion in real-time dashboards.",
  ...
  },
  {
    ...
    description: "Patients book, reschedule, and confirm 24/7 via voice or SMS. Directly syncs with your EHR.",
  ...
  },
  {
    ...
    description: "End-to-end encryption, SOC 2 Type II certified infrastructure. HL7 FHIR standards supported.",
    stats: "SOC 2 & HIPAA",
  ...
  },
  {
    icon: Zap,
    title: "Instant Setup",
    description: "Go live in under 48 hours. Connects with Epic, Cerner, Athenahealth, and 50+ healthcare systems.",
    stats: "< 48hr setup",
    ...
    <section className="py-24 lg:py-32 section-alt relative overflow-hidden" id="demos">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="text-center mb-14">
          <p className="text-sm font-semibold text-accent tracking-wide mb-3">Platform</p>
          <h2 className="font-display font-bold text-3xl sm:text-4xl md:text-5xl text-foreground mb-4">
            Everything Your Organization Needs
          </h2>
          <p className="text-muted-foreground max-w-xl mx-auto text-sm">
            A complete AI-powered suite designed for modern healthcare operations.
          </p>
        </motion.div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {demos.map((demo, i) => (
            <motion.div key={demo.title} custom={i} variants={cardVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} whileHover={{ y: -4, transition: { duration: 0.25 } }}
              ...
              <div className="relative h-36 overflow-hidden">
                <img src={demo.image} alt={demo.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
              ...
              <div className="p-6 pt-4">
                <h3 className="font-display font-bold text-base text-foreground mb-2">{demo.title}</h3>
                <p className="text-sm text-muted-foreground leading-relaxed mb-4">{demo.description}</p>
                <span className="inline-block text-xs font-semibold text-accent bg-accent/10 rounded-full px-3 py-1">{demo.stats}</span>
              </div>
            </motion.div>
          ))}
        </div>
