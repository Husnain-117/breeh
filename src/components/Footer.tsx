...
const Footer = ({ onOpenPlaybook, onBookDemo }: FooterProps) => {
  ...
  const footerLinks = {
    ...
    Resources: [
      ...
      { label: "ROI Calculator", isNew: true, href: "/roi-calculator" },
    ],
  };
                ...
                {links.map((link) => (
                  <li key={link.label}>
                    {"href" in link && link.href ? (
                      <Link
                        href={link.href}
                        className="text-sm text-primary-foreground/50 hover:text-primary-foreground transition-colors duration-300 inline-flex items-center gap-2"
                      >
                        {link.label}
        ...
        <div className="mt-16 pt-8 border-t border-primary-foreground/10 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex flex-wrap gap-6">
            {["Terms of Service", "Privacy Policy", "HIPAA Compliance"].map((link) => (
              <a key={link} href="#" className="text-xs text-primary-foreground/40 hover:text-primary-foreground transition-colors duration-300">{link}</a>
            ))}
          </div>
